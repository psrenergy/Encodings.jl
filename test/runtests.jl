...
@testset "Encoding Error + Fallback :: ISO-LATIN-1" begin
    @test_throws EncodingError encode(text, EncodingMachine.ISO_LATIN_1())
    @test encode(text, EncodingMachine.ISO_LATIN_1(), UInt8('?')) == data
end
...
@testset "Decoding Error + Fallback :: ISO-LATIN-1" begin
    @test_throws DecodingError decode(data, EncodingMachine.ISO_LATIN_1())
    @test decode(data, EncodingMachine.ISO_LATIN_1(), Char(0x3f)) == text
end